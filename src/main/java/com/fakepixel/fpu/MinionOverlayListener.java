    ...
    public void onGuiDraw(GuiScreenEvent.DrawScreenEvent.Post event) {
        ...
        for (int i = 0; i < lowerChestInventory.getSizeInventory(); i++) {
            ...
            if (slotItem != null && slotItem.getItem() != null) {
                ...
                if (cleanItemName.contains("Diamond Spreading") || cleanItemName.contains("Compactor") || cleanItemName.contains("Bucket") || cleanItemName.contains("Wheel") || cleanItemName.contains("Foul Flesh") || cleanItemName.contains("Catalyst")) isUpgradeOrUI = true;
            ...
            for (Map.Entry<String, Integer> entry : itemQuantities.entrySet()) {
                ...
                if (data != null && data.bzSell != null && !data.bzSell.equals("N/A")) {
                    ...
                    try {
                        ...
                        dynamicItemLines.add(EnumChatFormatting.GRAY + " -> Total: " + EnumChatFormatting.GREEN + String.format("%,.1f", totalVal) + " c");
        ...
        int totalItems = dynamicItemLines.size() / 4; 
        int dWheel = Mouse.getDWheel();
        if (dWheel > 0) scrollOffset--; else if (dWheel < 0) scrollOffset++;
        
        if (scrollOffset < 0) scrollOffset = 0;
        if (scrollOffset > Math.max(0, totalItems - MAX_VISIBLE_ITEMS)) {
        ...
        if (showPrices) {
            ...
            if (!PriceManager.isServerOnline) {
                ...
                profitLines.add(EnumChatFormatting.DARK_GRAY + "Bypassing server lag cleanly.");
            } else if (!dataTracked) {
                profitLines.add(EnumChatFormatting.RED + "No price data synced.");
                profitLines.add(EnumChatFormatting.GRAY + "Hover items in Bz once.");
            } else {
                int startIdx = scrollOffset * 4;
                int endIdx = Math.min(startIdx + (MAX_VISIBLE_ITEMS * 4), dynamicItemLines.size());
            ...
            profitLines.add(EnumChatFormatting.GOLD + "--------------------");
            profitLines.add(EnumChatFormatting.AQUA + "Total Value: " + EnumChatFormatting.GREEN + String.format("%,.1f", totalMinionCoins));

            int boxWidth = 165;
            int boxHeight = profitLines.size() * 11 + 8;
            GuiChest.drawRect(rightX, y, rightX + boxWidth, y + boxHeight, 0xF5101014);
            int currentY = y + 5;
        ...
        if (showAdvice) {
            ...
            if (chestTitle.contains("Snow") || chestTitle.contains("Clay") || chestTitle.contains("Slime")) {
                if (!hasDiamondSpreading) adviceLines.add(EnumChatFormatting.RED + " -> Missing Dia Spreading");
            ...
            GuiChest.drawRect(leftX, y, leftX + boxWidth, y + boxHeight, 0xF5101014);
            int currentY = y + 5;
